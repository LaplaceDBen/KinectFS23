...
logPath = r"C:\Users\rapha\OneDrive\Desktop\CDS_FS23\Projektarbeit\GitHubNew\KinectFS23\blender\qr_codes_test.log"      # Test-Logfile
...
class FollowLogThread(threading.Thread):
    ...
    def run(self):
        newline = subprocess.Popen(['powershell', 'Get-Content', logPath, '-Wait', '-Tail 1'],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        while not self.stopped():
            line = newline.stdout.readline()
            line = str(line).strip("b'").strip("\\r\\n'")
            if len(line) > 5:
                self.callback(line)
